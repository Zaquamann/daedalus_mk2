#!/usr/bin/env python3
"""Audio-only baseline on the filtered AV partition with Gaussian noise
(std=0.3) on the mel during training only. Validation stays clean."""
...
MODEL_PATH = os.path.join(SCRIPT_DIR, "models", "audio_only_noisy_filtered.pt")
CURVE_PNG = os.path.join(SCRIPT_DIR, "analysis", "audio_only_noisy_filtered_curves.png")
CURVE_CSV = os.path.join(SCRIPT_DIR, "analysis", "audio_only_noisy_filtered_curves.csv")
...
TEST_SIZE = 0.33
RANDOM_SEED = 42
NOISE_STD = 0.3       # Gaussian std added to mel at TRAIN time only
...
def main() -> None:
    ...
    for epoch in range(1, NUM_EPOCHS + 1):
        ...
        for X_batch, y_batch in train_loader:
            ...
            # Gaussian noise on mel (training only).
