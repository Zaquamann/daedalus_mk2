...
# D315 cause A: gate-supervision weight. Teach the per-trial routing the average
# loss under-weights — on BOTH-present DECISIVE trials push w_a->0.9 when video is
# wrong & audio right (the clean-confusable E1d regime), ->0.1 when audio wrong &
# video right. Converts the present-but-underused conf_v signal into the needed swing.
GATE_SUP_W = float(os.environ.get("GATE_SUP_W", "1.0"))
# D320 candidate-e (tunable-vs-structural arbiter): detached-argmax target for the
# route_aud trials (video-wrong & audio-right). Default 0.9 reproduces (d); (e) sets
# 0.95 with GATE_SUP_W=4.0 to push the per-trial lever as hard as is sane — if E1d-clean
# discrimination STILL doesn't develop, the gate failure is STRUCTURAL, not tunable.
ROUTE_AUD_TGT = float(os.environ.get("ROUTE_AUD_TGT", "0.9"))
# D315 cause B: standalone audio model used to INIT + FREEZE the fused audio branch
# (the "A" model in analyze_av_msi; acc 0.903 / d' 4.61) so joint training cannot
# degrade the fused audio head below standalone (the convex combo cannot beat its
# own audio head, which capped E1d gL at 0.938 even with a perfect gate).
...
def main() -> None:
    ...
    print(f"seed={RANDOM_SEED}  epochs={NUM_EPOCHS}  aux_w={AUX_WEIGHT}  "
          f"gate_sup_w={GATE_SUP_W}  route_aud_tgt={ROUTE_AUD_TGT}  "
    ...
    print(f"OUT_TAG='{OUT_TAG}'  (D320 candidate-e: (d) + GATE_SUP_W={GATE_SUP_W} "
          f"& route_aud target={ROUTE_AUD_TGT} — tunable-vs-structural arbiter; "
          f"frozen audio (B) + 262-d confidence gate, else == (d))")
    ...
    model = AVLateFusionReliabilityWordResNet(
        num_classes, use_mid_gate=USE_MID_GATE)
    # D315 cause B: init the audio branch from the standalone A model and FREEZE it
    # (requires_grad=False here + BN eval in the loop) so the fused audio head keeps
    # standalone quality (acc 0.903 / d' 4.61). The label_to_idx alignment is asserted
    # because audio_fc is class-indexed — a mismatch would silently misroute logits.
    _a_ckpt = torch.load(A_CKPT, map_location="cpu", weights_only=False)
    assert _a_ckpt["label_to_idx"] == label_to_idx, (
        "standalone-A label_to_idx != AV label_to_idx — audio_fc would be misaligned")
    model.load_audio_pretrained(_a_ckpt["model_state_dict"])
    for _m in (model.audio_block1, model.audio_block2, model.audio_fc):
        for _p in _m.parameters():
            _p.requires_grad = False
    model = model.to(device)
    n_frozen = sum(p.numel() for p in model.parameters() if not p.requires_grad)
    n_trainable = sum(p.numel() for p in model.parameters() if p.requires_grad)
    print(f"D315-B: audio branch init from {os.path.basename(A_CKPT)} + FROZEN  "
    ...
    # D315-B: optimize only the unfrozen params (visual + rel_gate); the frozen audio
    # branch is excluded so AdamW weight-decay never touches it either.
    ...
    for epoch in range(1, NUM_EPOCHS + 1):
        ...
        model.train()
        # D315-B: keep the FROZEN audio branch in eval mode so its BatchNorm running
        # stats stay at the standalone-A values (don't drift toward the noisy fused
        # batch stats) — the audio head must remain the standalone d'=4.61 readout.
        model.audio_block1.eval()
        model.audio_block2.eval()
        ...
        for mel, video, y in train_loader:
            ...
            # D315 cause A — GATE SUPERVISION (fp32, outside autocast for BCE safety).
            # On BOTH-present DECISIVE trials (exactly one head correct), push w_a
            # toward audio (0.9) when video is wrong & audio right — the clean-
            # confusable E1d regime the average loss under-weights — and toward video
            # (0.1) when audio wrong & video right. both-right/both-wrong -> no push
            # (gate stays free to learn ~0.5 for the optimal equal-reliability combo).
            # Targets are DETACHED head argmax-correctness (no grad into the heads).
            w_a = w[:, 0].float()
            a_ok = (la.detach().argmax(1) == y)
            v_ok = (lv.detach().argmax(1) == y)
            both_present = a_present & v_present
            route_aud = both_present & a_ok & (~v_ok)
            route_vid = both_present & v_ok & (~a_ok)
            sel = route_aud | route_vid
            if sel.any():
                tgt = torch.full_like(w_a, 0.1)
                tgt[route_aud] = ROUTE_AUD_TGT
                gate_loss = F.binary_cross_entropy(
                    w_a[sel].clamp(1e-6, 1.0 - 1e-6), tgt[sel])
                loss = loss + GATE_SUP_W * gate_loss
            loss.backward()
            optimizer.step()
