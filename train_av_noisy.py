#!/usr/bin/env python3
"""AV-fused training with Gaussian noise on the audio mel during training only
(visual stays clean). Validation uses clean audio + clean video so val_acc is
comparable to the clean AV model."""
...
MODEL_PATH = os.path.join(SCRIPT_DIR, "models", "av_fused_noisy.pt")
CURVE_PNG = os.path.join(SCRIPT_DIR, "analysis", "av_fused_noisy_curves.png")
CURVE_CSV = os.path.join(SCRIPT_DIR, "analysis", "av_fused_noisy_curves.csv")
...
NOISE_STD = 0.3       # Gaussian std added to audio mel at TRAIN time only
...
def main() -> None:
    ...
    print(f"audio noise std (train only): {NOISE_STD}")
    ...
    for epoch in range(1, NUM_EPOCHS + 1):
        ...
        for mel, video, y in train_loader:
            ...
            # Audio-only Gaussian noise (training only). Visual stays clean.
