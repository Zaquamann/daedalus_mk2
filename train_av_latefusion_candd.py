...
def main() -> None:
    ...
    print(f"seed={RANDOM_SEED}  epochs={NUM_EPOCHS}  aux_w={AUX_WEIGHT}  "
          f"gate_sup_w={GATE_SUP_W}  mid_gate={USE_MID_GATE}  workers={NUM_WORKERS}")
    print(f"modality dropout: P(audio-dead)={P_ADEAD:.2f}  "
          f"P(video-dead)={P_VDEAD:.2f}  P(both)={1-P_ADEAD-P_VDEAD:.2f}")
    print(f"OUT_TAG='{OUT_TAG}'  (D315 candidate-d: (c) confidence gate + A "
          f"gate-supervision + B audio branch init-from-A & FROZEN; else == GO)")
