#!/usr/bin/env python3
"""Train the early-fusion AV variant (D3.1). Same training recipe as
`train_av.py`; only the fusion stage (at input rather than mid-block) differs."""
...
MODEL_PATH = os.path.join(SCRIPT_DIR, "models", "av_fused_early.pt")
CURVE_PNG = os.path.join(SCRIPT_DIR, "analysis", "deepdive",
                          "D3_retrain_early_curves.png")
CURVE_CSV = os.path.join(SCRIPT_DIR, "analysis", "deepdive",
                          "D3_retrain_early_log.csv")
...
def main() -> None:
    ...
    model = AVEarlyFusionWordResNet(num_classes).to(device)
    n_total = sum(p.numel() for p in model.parameters())
    n_fc = sum(p.numel() for p in model.fc.parameters())
    print(f"Model parameters: total={n_total:,}, fc={n_fc:,}")
    ...
    for epoch in range(1, NUM_EPOCHS + 1):
        ...
        if val_acc > best_val_acc:
            ...
            torch.save({
                ...
                "architecture": "early_fusion_d3_1",
            }, MODEL_PATH)
        scheduler.step()
