...
def _conf(logit: torch.Tensor) -> torch.Tensor:
    """Per-head logit-CONFIDENCE features for the reliability gate (D313 cand-c).

    The gate's penult inputs encode word IDENTITY, not reliability — on
    "is this E1d video wrong?" they decode at chance (~0.47). The per-trial
    reliability lives in each head's logit confidence: [entropy, max-prob,
    top1-top2 margin], which decode the routing target at ~0.91 (~0.76 on the
    E1d-specific question, D314 probe). Computed from DETACHED logits so the gate
    estimates reliability without distorting the readouts. (B, C) -> (B, 3).
    """
    p = logit.softmax(1)
    ent = -(p * p.clamp_min(1e-12).log()).sum(1, keepdim=True)
    top2 = p.topk(2, dim=1).values
    return torch.cat([ent, top2[:, :1], top2[:, :1] - top2[:, 1:2]], dim=1)
...
class AVLateFusionReliabilityWordResNet(nn.Module):
    ...
    def __init__(self, num_classes: int, alpha_init: float = 0.2,
        ...
        self.rel_gate = nn.Sequential(
            # D313 candidate-(c): the gate also reads per-head logit-confidence
            # (3 audio + 3 video) on top of the two 128-d penults — the penults
            # decode reliability at chance, the confidence carries the routing signal.
            nn.Linear(128 + 128 + 6, 64),   # a_pen(128)+v_pen(128)+conf_a(3)+conf_v(3)
    ...
    def forward(
        ...
        # Reliability weights from (detached) penults PLUS per-head logit-confidence
        # (D313 candidate-c). The penults give word identity but decode per-trial
        # reliability at chance; each head's [entropy, max-prob, top1-top2 margin]
        # carries the reliability the gate must route on. Detached: estimate, don't
        # distort. Penults KEPT — the dead-stream detector (video-dead -> w_a=1) needs
        # v_pen -> 0; confidence is ADDED, not a replacement.
        conf_a = _conf(logit_a.detach())                            # (B, 3)
        conf_v = _conf(logit_v.detach())                            # (B, 3)
        rel_in = torch.cat([a_pen.detach(), v_pen.detach(),
                            conf_a, conf_v], dim=1)                  # (B, 262)
