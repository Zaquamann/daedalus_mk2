#!/usr/bin/env python3
"""Train the additive-gate AV variant (D3.10). Same training recipe as
`train_av.py`; only the gate's combination rule differs."""
...
MODEL_PATH = os.path.join(SCRIPT_DIR, "models", "av_fused_additive.pt")
CURVE_PNG = os.path.join(SCRIPT_DIR, "analysis", "deepdive",
                          "D3_retrain_additive_curves.png")
CURVE_CSV = os.path.join(SCRIPT_DIR, "analysis", "deepdive",
                          "D3_retrain_additive_log.csv")
...
def _save_curves(history: dict, num_classes: int, best_val_acc: float) -> None:
    ...
    ax.set_title(f"Accuracy (best val: {best_val_acc:.1%}, "
...
def _write_curves_csv(history: dict) -> None:
    ...
    with open(CURVE_CSV, "w") as f:
        f.write("epoch,train_loss,train_acc,val_loss,val_acc,"
                 "alpha,epoch_time_s,peak_gpu_gib\n")
...
def main() -> None:
    ...
    model = AVAdditiveWordResNet(num_classes, alpha_init=ALPHA_INIT).to(device)
    ...
    print(f"Model parameters: total={n_total:,}, "
          f"visual={n_visual:,}, gate={n_gate:,}")
    print(f"α init: {model.gate.alpha.item():.4f}")
    print(f"video t_stride: {T_STRIDE}")
    print(f"bf16 autocast: {USE_BF16} | torch.compile: {USE_COMPILE}")
    ...
    for epoch in range(1, NUM_EPOCHS + 1):
        ...
        if val_acc > best_val_acc:
            ...
            torch.save({
                ...
                "architecture": "additive_gate_d3_10",
            }, MODEL_PATH)
        scheduler.step()
